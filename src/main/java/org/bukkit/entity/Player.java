...
public interface Player extends HumanEntity, Conversable, CommandSender, OfflinePlayer, PluginMessageRecipient {
     ...
     * will automatically switch to it once the download is complete. If the client has downloaded
     * and cached the same resource pack in the past, it will perform a quick timestamp check over
     * the network to determine if the resource pack has changed and needs to be downloaded again.
     * When this request is sent for the very first time from a given server, the client will first
     * display a confirmation GUI to the player before proceeding with the download.
     * <p>
     * Notes:
     *   <ul>
     *     <li>Players can disable server resources on their client, in which case this method will have no affect on them.</li>
     *     <li>There is no concept of resetting resource packs back to default within Minecraft, so players will have to relog to do so.</li>
     *   </ul>
     *
     * @param url The URL from which the client will download the texture pack. The string must contain
     * only US-ASCII characters and should be encoded as per RFC 1738.
